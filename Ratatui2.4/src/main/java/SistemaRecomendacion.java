        ...
        for (Condicion condicion : usuario.condiciones) {
            switch (condicion.getNombre()) {
                ...
                case "hipertension":
                    ...
                    break;
                case "vegano":
                    if (!receta.isVegan()) { // Verifica la propiedad vegana de la receta
                        aprobada = false;
                    }
                    break;
                case "vegetariano":
                    if (!receta.isVegetarian()) { // Verifica la propiedad vegetariana de la receta
                        aprobada = false;
                    }
                    break;
                case "dairyfree":
                    if (!receta.isLactosa()) {  // Asumiendo que Receta tiene este método
                        aprobada = false;
                    }
                    break;
